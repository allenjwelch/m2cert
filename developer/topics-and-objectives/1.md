...
\n Magento 2 FPC, pages are either cached or they are not. To make a page
not cacheable, you can add the cacheable="false" attribute to any block
on the page. Keep in mind that this affects website performance and should be
used sparingly.
Combining speed and personalization involves serving a cached page and then
substituting or adding content with an AJAX request.
The DevDocs contains an article on how to do this with UI Components. This is
considered the only “proper” way of hole punching as Magento leverages the
browser’s local cache storage system.
...
You can clear the cache programmatically by calling the \Magento\Framework\
App\CacheInterface::remove() method.
**_References:_**
